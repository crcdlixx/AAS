...
const cache = new Map<string, { models: string[]; expiresAtMs: number }>()
...
export async function fetchModelListFromApi(options: FetchModelListOptions): Promise<string[]> {
  ...
  try {
    ...
    const filtered = filterModelsForChat(extracted)

    cache.set(key, { models: filtered, expiresAtMs: now + CACHE_TTL_MS })
    return filtered
  } finally {
